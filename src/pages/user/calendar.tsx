...
function getFacilityColor(facility: any, fallbackIndex: number) {
  const name = (facility?.name || "").toLowerCase();

  if (name.includes("collaboration zone b")) return FACILITY_COLORS[0];
  if (name.includes("creative design studio")) return FACILITY_COLORS[1];
  if (name.includes("executive meeting room")) return FACILITY_COLORS[4];
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    const fetchFacilities = async () => {
      try {
        ...
        const data = response.data.data?.facilities || response.data.data || [];
  ...
  // Map facility ID → color index
  const facilityColorMap: Record<string, number> = {};

  facilities.forEach((f, i) => {
    facilityColorMap[f.id] = i % FACILITY_COLORS.length;
  });
  ...
  const selectedColor = getFacilityColor(
    selectedFacilityData,
    selectedColorIdx,
  );

  // Count recurring group sizes
  ...
  // Deduplicate by ID only (API safety net), keep ALL dates visible
  ...
  const compactedByDate = Object.fromEntries(
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <aside className="w-full lg:w-52 shrink-0 flex flex-col gap-3">
          ...
          <div className="flex flex-col gap-1.5">
            {loading ? (
              <div className="animate-pulse flex flex-col gap-2">
                {[1, 2, 3, 4].map((i) => (
                  <div key={i} className="h-14 bg-default-100 rounded-2xl" />
                ))}
              </div>
            ) : (
              facilities.map((f, idx) => {
                const color = getFacilityColor(f, idx);
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${
                        isSelected
                          ? `${color.activeBg} text-white border-transparent shadow-lg ${color.activeShadow}`
                          : `bg-background ${color.border} hover:${color.bg} hover:border-opacity-60`
                      }
                    `}
          ...
          {!loading && facilities.length > 0 && (
            <div className="mt-2 p-3 bg-default-50 rounded-2xl border border-default-100">
            ...
            >
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-4 min-w-0">
                  <div
                    className={`p-3 ${selectedColor.bg} rounded-2xl ${selectedColor.text} shrink-0`}
                  >
                    <Calendar size={22} />
                  </div>
            ...
            <Card.Content className="p-0">
              {bookingsLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 text-center px-8">
                  ...
                  <Button
                    className="mt-6 h-11 rounded-2xl font-bold px-8"
                    variant="primary"
                ...
                <div className="flex flex-col">
                  {Object.entries(compactedByDate).map(([date, daySlots]) => (
                    <div key={date}>
                      {/* Date header */}
                      <div
                        className={`px-6 py-2.5 border-y border-default-100 flex items-center justify-between gap-2 ${selectedColor.bg}`}
                      >
                        <div className="flex items-center gap-2">
                          <span
                            className={`w-1.5 h-1.5 rounded-full shrink-0 ${selectedColor.dot}`}
                          />
                          <span className="text-[10px] font-black text-default-600 uppercase tracking-widest">
                            {date}
                          </span>
                        </div>
                        <span
                        ...
                        >
                          {daySlots.reduce(
                            (total, slot: any[]) => total + slot.length,
                            0,
                          )}{" "}
                        ...
                        return (
                          <div
                            ...
                            className={`
                              ...
                              px-6 py-4 border-b border-default-50 last:border-0
                              hover:bg-default-50/50 transition-colors duration-150
                              ${isLive ? `border-l-4 ${selectedColor.strip} ${selectedColor.bg}` : ""}
                            `}
                            title={slotTitle}
                          >
                            {/* Left accent bar */}
