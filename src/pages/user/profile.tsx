...
export default function ProfilePage() {
  ...
  return (
    <div className="max-w-2xl mx-auto py-12 px-4">
      ...
      <Card className="p-10 border border-default-200 rounded-[3rem] bg-surface/60 backdrop-blur-xl shadow-2xl shadow-default-100">
        <form className="flex flex-col gap-8" onSubmit={handleUpdate}>
          ...
          {error && (
            <div className="bg-danger/10 text-danger p-4 rounded-2xl border border-danger/20 flex items-center gap-3 font-bold">
              <AlertCircle size={18} />
              {error}
            </div>
          )}
          {success && (
            <div className="bg-success/10 text-success p-4 rounded-2xl border border-success/20 flex items-center gap-3 font-bold">
          ...
          <div className="space-y-6">
            <TextField isDisabled name="email" type="email">
              ...
              <InputGroup className="rounded-2xl bg-default-100/50">
          ...
          <Button
            className="w-full h-16 rounded-[1.5rem] font-black text-lg shadow-xl shadow-primary/30 mt-4 transition-all active:scale-95"
            isPending={updating}
            type="submit"
            variant="primary"
          >
